...
export interface TaskEvent {
  type: 'role-start' | 'role-complete' | 'thought' | 'code-change' | 'file-create' | 'status-update' | 'summary' | 'error' | 'log';
  timestamp: string;
...
// JSON endpoint — returns events as JSON (for polling)
export async function handleEventsJson(
  env: Env,
  requestId: string
): Promise<Response> {
  const events = await getEvents(env, requestId);
  return new Response(JSON.stringify({ success: true, data: { events, count: events.length } }), {
    status: 200,
    headers: {
...
export async function handleEventPost(
  ...
  let body: { request_id: string; event: TaskEvent } | { request_id: string; events: TaskEvent[] };
  try {
  ...
  if (body.event) {
    await appendEvent(env, request_id, body.event);
